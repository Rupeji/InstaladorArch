...
# SOLUCIÓN AL ERROR DE BIND: Asignar la IP en caliente de inmediato
echo "--> Activando IP fija global temporal en la interfaz $INTERFACE..."
if ! ip addr show dev "$INTERFACE" | grep -q "${LANCACHE_IP}/24"; then
    sudo ip addr add "${LANCACHE_IP}/24" dev "$INTERFACE" || echo "Aviso: No se pudo asignar la IP (puede que ya exista)."
fi
...
# Generar docker-compose.yml con LanCache y la nueva GUI integrada y adaptada
