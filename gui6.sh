...
# MODIFICADO: DNS_BIND_IP cambiado a 0.0.0.0 para resolver el conflicto de interfaces internas y permitir respuestas DNS externas
...
# MODIFICADO: Apuntar el host a su IP física ($LANCACHE_IP) ya que el DNS ahora sí responde correctamente en esa interfaz externo
sudo rm -f /etc/resolv.conf
cat <<EOF | sudo tee /etc/resolv.conf > /dev/null
...
echo "-------------------------------------------------------"
echo " NOTA DE RED:"
echo " Dado que gestionas la IP fija (DHCP estático) en tu"
echo " router, asegúrate de que la reserva para la MAC"
echo " de este equipo esté guardada antes de reiniciar."
echo ""
